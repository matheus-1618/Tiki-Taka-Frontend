...
export default function Eredvise(props) {
  const [teams, setTeams] = useState([]);
  const [isLoading, setIsLoading] = useState(true); 
  const eredvise = (<>
      <br></br>
      <br></br>
      <h1 className="letra">EREDVISE 2022</h1>
      <br></br>
      <div className="table-container">
        <div className="eredvise">
          <h1>Posição</h1>
          <h1></h1>
          <h1></h1>
          <h1></h1>
  ...
  useEffect(() => {
    axios
      .get("https://sleepy-wave-09569.herokuapp.com/eredvise")
  ...
  return (
    <main className="App">
      <Appbar/>
      {isLoading ? <LoadingSpinner/> : eredvise}
      <Bottom/>
    </main>
  );
}