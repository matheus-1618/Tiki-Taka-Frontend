...
export default function Liganos(props) {
  const [teams, setTeams] = useState([]);
  const [isLoading, setIsLoading] = useState(true); 
  const liganos = (<>
      <br></br>
      <br></br>
      <h1 className="letra">LIGA NOS 2022</h1>
      <br></br>
      <div className="table-container">
        <div className="liganos">
          <h1>Posição</h1>
          <h1></h1>
          <h1></h1>
          <h1></h1>
  ...
  useEffect(() => {
    axios
      .get("https://sleepy-wave-09569.herokuapp.com/liganos")
  ...
  return (
    <main className="App">
      <Appbar/>
      {isLoading ? <LoadingSpinner/> : liganos}
      <Bottom/>
    </main>
  );
}