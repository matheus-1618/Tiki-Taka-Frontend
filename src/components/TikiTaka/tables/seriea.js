...
export default function SerieA(props) {
  const [teams, setTeams] = useState([]);
  const [isLoading, setIsLoading] = useState(true); 
  const seriea = (<>
      <br></br>
      <br></br>
      <h1 className="letra">SERIE A TIM 2022</h1>
      <br></br>
  ...
  useEffect(() => {
    axios
      .get("https://sleepy-wave-09569.herokuapp.com/seriea")
  ...
  return (
    <main className="App">
      <Appbar/>
      {isLoading ? <LoadingSpinner/> : seriea}
      <Bottom/>
    </main>
  );
}